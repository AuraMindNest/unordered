...
否则，当且仅当容器中不存在等价键的元素时，插入 `nh` 拥有的元素。

[horizontal]
要求：;; `nh` 为空或 `nh.get_allocator()` 等于容器的分配器。
返回：;; 若 `nh` 为空，则返回的 `insert_return_type` 包含：`inserted` 为 `false`，`position` 等于 `end()`，`node` 为空。
+
否则，若已存在等价键的元素，则返回的 `insert_return_type` 包含：`inserted` 为 `false`，`position` 指向匹配元素，`node` 包含来自 `nh` 的节点。
+
否则，若插入成功，则返回的 `insert_return_type` 包含：`inserted` 为 `true`，`position` 指向新插入的元素，`node` 为空。
抛出：;; 若异常由调用 `hasher` 之外的操作引发，则该函数无任何效果。
备注：;; 可能使迭代器失效，但仅当插入导致负载因子大于或等于最大负载因子时发生。
+
指向元素的指针和引用永不失效。
...
尝试通过迭代 `source` 并提取 `source` 中不包含在 `*this` 中的任何节点，然后将其插入 `*this` 来“合并”两个容器。
