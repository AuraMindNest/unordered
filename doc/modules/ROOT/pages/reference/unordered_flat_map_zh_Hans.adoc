...
若插入成功，则迭代器指向新插入的元素；否则，指向具有等价键的元素。
抛出;; 若异常由 `hasher` 调用之外的操作引发，则该函数不产生任何效果。
注意;; 此函数与 xref:#unordered_flat_map_emplace[emplace] 类似，区别在于：若已存在具有等效键的元素，则不会构造任何 `value++_++type` 对象；否则，将按以下形式构造： +
+
--
```c++
...
抛出;; 若异常由 `hasher` 调用之外的操作引发，则该函数不产生任何效果。
注意;; 此函数与 xref:#unordered_flat_map_emplace_hint[emplace++_++hint] 类似，区别在于：若已存在具有等效键的元素，则不会构造任何 `value++_++type` 对象；否则，将按以下形式构造： +
+
--
```c++
...
注意;; 仅当 xref:reference/stats.adoc#stats[统计计算] 功能 xref:#unordered_flat_map_boost_unordered_enable_stats[已启用] 时可用。
