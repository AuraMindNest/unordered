...
由于使用了开放寻址，`boost::unordered_flat_map` 的接口在多个方面与 `boost::unordered_map`/`std::unordered_map` 有所不同：
  ...
  - 在重哈希过程中不保持指针稳定性。
  - `begin()` 不是常数时间操作。
  - 没有用于桶处理（除了 `bucket_count`）或节点提取/插入的 API。
  - 容器的最大负载因子由内部管理，不能由用户设置。
...
|_Allocator_
|一种分配器，其值类型与容器的值类型相同。
支持使用 https://en.cppreference.com/w/cpp/named_req/Allocator#Fancy_pointers[花式指针(fancy pointers)] 的分配器。

|===

容器的元素存放在内部的 _桶数组_ 中。元素被插入到由其哈希码确定的桶中，但如果该桶已被占用（发生 _冲突_），则使用原始位置附近的一个可用桶。

桶数组的大小可通过调用 `insert`/`emplace` 自动增加，或作为调用 `rehash`/`reserve` 的结果而增加。容器的 _负载因子_（元素数量除以桶数量）永远不会大于 `max_load_factor()`，除非在小型容器中实现可能允许更高的负载。
...
使用 `hf` 作为哈希函数、`eql` 作为键相等谓词、`a` 作为分配器，构造一个至少包含 `n` 个桶的空容器。
...
使用 `hf` 作为哈希函数、`eql` 作为键相等谓词、`a` 作为分配器，构造一个至少包含 `n` 个桶的空容器，并将 `[f, l)` 范围内的元素插入其中。
...
使用 `a` 作为分配器，以及默认的哈希函数和键相等谓词，构造一个空容器，并将 `[f, l)` 范围内的元素插入其中。
...
构造一个容器，复制 `other` 包含的元素、哈希函数和谓词，但使用分配器 `a`。
...
使用 `hf` 作为哈希函数、`eql` 作为键相等谓词、`a` 作为分配器，构造一个至少包含 `n` 个桶的空容器，并将 `il` 中的元素插入其中。
...
使用默认哈希函数、默认键相等谓词和 `a` 作为分配器，构造一个至少包含 `n` 个桶的空容器。
...
使用 `hf` 作为哈希函数、默认键相等谓词和 `a` 作为分配器，构造一个至少包含 `n` 个桶的空容器。
...
使用 `a` 作为分配器、默认哈希函数和默认键相等谓词，构造一个至少包含 `n` 个桶的空容器，并将 `[f, l)` 范围内的元素插入其中。
...
使用 `hf` 作为哈希函数、`a` 作为分配器、默认键相等谓词，构造一个至少包含 `n` 个桶的空容器，并将 `[f, l)` 范围内的元素插入其中。
...
使用 `a` 作为分配器、默认哈希函数和默认键相等谓词，构造一个空容器，并将 `il` 中的元素插入其中。
...
使用 `a` 作为分配器、默认哈希函数和默认键相等谓词，构造一个至少包含 `n` 个桶的空容器，并将 `il` 中的元素插入其中。
...
使用 `hf` 作为哈希函数、`a` 作为分配器、默认键相等谓词，构造一个至少包含 `n` 个桶的空容器，并将 `il` 中的元素插入其中。
...
[horizontal]
返回：;; 一个指向容器第一个元素的 `const_iterator`，若容器为空则为容器的尾后值。
复杂度：;; O(`bucket_count()`)
...
当且仅当容器中不存在等价键时，使用参数 `args` 构造一个对象并插入容器。

[horizontal]
要求：;; `value_type` 必须可从 `args` 构造。
返回：;; 若插入发生，则返回类型的 `bool` 分量为 `true`。+
+
若插入发生，则迭代器指向新插入的元素；否则，指向具有等价键的元素。
...
将一系列元素插入容器。当且仅当容器中不存在等价键时，元素才会被插入。

[horizontal]
要求：;; `value_type` 必须可从 `*first` https://en.cppreference.com/w/cpp/named_req/EmplaceConstructible[原地构造(EmplaceConstructible)^] 到容器中。
抛出：;; 插入单个元素时，若由调用 `hasher` 以外的操作引发异常，则函数无效果。
注意：;; 可能使迭代器、指针和引用失效，但仅当插入导致负载超过最大负载时。
...
[horizontal]
要求：;; `value_type` 必须 https://en.cppreference.com/w/cpp/named_req/CopyInsertable[可复制插入(CopyInsertable)^] 到容器中。
抛出：;; 插入单个元素时，若由调用 `hasher` 以外的操作引发异常，则函数无效果。
注意：;; 可能使迭代器、指针和引用失效，但仅当插入导致负载超过最大负载时。
...
仅当 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员类型定义，且 `iterator` 和 `const_iterator` 均不能从 `K` 隐式转换时，`template<class K, class\... Args>` 重载才参与重载决议。库假定 `Hash` 可同时用 `K` 和 `Key` 调用，且 `Pred` 是透明的。这支持异构查找，避免了实例化 `Key` 类型的开销。
...
[horizontal]
返回：;; 若插入发生，则迭代器指向新插入的元素；否则，指向具有等价键的元素。
抛出：;; 若由调用 `hasher` 以外的操作引发异常，则函数无效果。
...
注意：;; 仅当 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员类型定义，且 `iterator` 和 `const_iterator` 均不能从 `K` 隐式转换时，`template<class K>` 重载才参与重载决议。库假定 `Hash` 可同时用 `K` 和 `Key` 调用，且 `Pred` 是透明的。这支持异构查找，避免了实例化 `Key` 类型的开销。
...
抛出：;; 在此实现中无效果（既不调用 `hasher` 也不调用 `key_equal` 对象）。
...
从 `position` 指向的元素移动构造一个 `init_value` `x`，擦除该元素并返回 `x`。
...
[horizontal]
返回：;; 指向键等价于 `k` 的元素的迭代器，若不存在此元素则为 `end()`。
...
[horizontal]
返回：;; 包含所有键等价于 `k` 的元素的范围。若容器不包含任何此类元素，则返回 `std::make_pair(b.end(), b.end())`。
注意：;; 仅当 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员类型定义时，`template<class K>` 重载才参与重载决议。库假定 `Hash` 可同时用 `K` 和 `Key` 调用，且 `Pred` 是透明的。这支持异构查找，避免了实例化 `Key` 类型的开销。
...
注意：;; 可能使迭代器、指针和引用失效，但仅当插入导致负载超过最大负载时。+
+
仅当 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员类型定义时，`template<class K>` 重载才参与重载决议。库假定 `Hash` 可同时用 `K` 和 `Key` 调用，且 `Pred` 是透明的。这支持异构查找，避免了实例化 `Key` 类型的开销。
...
[horizontal]
返回：;; 在不再擦除元素的前提下，容器在不重哈希的情况下可容纳的最大元素数。
...
必要时更改桶数组的大小，使其至少包含 `n` 个桶，且负载因子小于或等于最大负载因子。适用时，此操作将增加或减少与容器关联的 `bucket_count()`。
