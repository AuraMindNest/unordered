...
备注：;; 可能使迭代器、指针和引用失效，但仅当插入导致负载超过最大负载时发生。+
+
形式为 `insert(hint, x)` 的调用（其中 `x` 可同等地转换为 `const value_type&` 和 `const init_type&`）不歧义，并选择 `init_type` 重载。
...
备注：;; 可能使迭代器、指针和引用失效，但仅当插入导致负载超过最大负载时发生。+
+
形式为 `insert(hint, x)` 的调用（其中 `x` 可同等地转换为 `value_type&&` 和 `init_type&&`）不歧义，并选择 `init_type` 重载。
...
若插入发生，则迭代器指向新插入的元素；否则指向具有等价键的元素。
抛出：;; 若因调用 `hasher` 以外的操作抛出异常，则该函数无效果。
备注：;; 此函数类似于 xref:#unordered_flat_map_emplace[emplace]，区别在于若存在等价键的元素则不构造 `value_type`；否则，构造形式如下：+
+
--
```c++
...
`hint` 是插入元素的建议位置。本实现忽略此提示。

[horizontal]
返回：;; 若插入发生，则迭代器指向新插入的元素；否则指向具有等价键的元素。
抛出：;; 若因调用 `hasher` 以外的操作抛出异常，则该函数无效果。
备注：;; 此函数类似于 xref:#unordered_flat_map_emplace_hint[emplace_hint]，区别在于若存在等价键的元素则不构造 `value_type`；否则，构造形式如下：+
+
--
```c++
...
若插入发生，则迭代器指向新插入的元素；否则指向具有等价键的元素。
抛出：;; 若因调用 `hasher` 以外的操作抛出异常，则该函数无效果。
备注：;; 可能使迭代器、指针和引用失效，但仅当插入导致负载超过最大负载时。+
+
...
[horizontal]
效果：;; 若容器中尚未存在键等价于 `k` 的元素，则插入值 `std::pair<key_type const, mapped_type>(k, mapped_type())`。
...
备注：;; 仅当 xref:reference/stats.adoc#stats[统计计算] 被 xref:unordered_flat_map_boost_unordered_enable_stats[启用] 时可用。
