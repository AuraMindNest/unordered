...
测试对 `boost::concurrent++_++flat++_++map` 与 `boost::concurrent++_++node++_++map` 均采用了常规访问与 xref:concurrent.adoc#concurrent_bulk_visitation[批量访问] 两种模式：对于后者，查找键会先缓存在本地数组中，待缓冲区达到 xref:reference/concurrent_flat_map.adoc#concurrent_flat_map_constants[`bulk++_++visit++_++size`] 大小时再统一处理。
