...
返回：;; 若插入发生则为 `true`。
并发性：;; 阻塞于 `*this` 的重哈希。
说明：;; 仅当 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员类型定义时，此重载才会参与重载决议。库假定 `Hash` 可同时被 `K` 和 `Key` 调用，且 `Pred` 是透明的。这支持异构查找，避免了实例化 `Key` 类型的开销。
...
返回：;; 若插入发生则为 `true`。
并发性：;; 阻塞于 `*this` 的重哈希。
说明：;; 仅当 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员类型定义时，这些重载才会参与重载决议。库假定 `Hash` 可同时被 `K` 和 `Key` 调用，且 `Pred` 是透明的。这支持异构查找，避免了实例化 `Key` 类型的开销。
...
说明：;; 仅当 xref:reference/stats.adoc#stats[统计计算] xref:concurrent_node_set_boost_unordered_enable_stats[已启用] 时可用。
