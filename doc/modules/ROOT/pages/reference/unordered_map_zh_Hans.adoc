...
|_Key_
|`Key` 必须可从容器中 https://en.cppreference.com/w/cpp/named_req/Erasable[擦除^]（即 `allocator_traits` 可销毁它）。

|_T_
|`T` 必须可从容器中 https://en.cppreference.com/w/cpp/named_req/Erasable[擦除^]（即 `allocator_traits` 可销毁它）。

|_Hash_
|一种一元函数对象类型，用作 `Key` 的哈希函数。它接受一个类型为 `Key` 的参数，并返回一个类型为 `std::size_t` 的值。

|_Pred_
|一种二元函数对象，用于在类型为 `Key` 的值上实现等价关系。它接受两个类型为 `Key` 的参数，并返回一个布尔类型的值。
...
[source, c++, subs=+quotes]
----
typedef _实现定义_ local_iterator;
----

一个与迭代器具有相同值类型、差异类型、指针和引用类型的迭代器。
...
[source, c++, subs=+quotes]
----
typedef _实现定义_ const_local_iterator;
----

一个与 const_iterator 具有相同值类型、差异类型、指针和引用类型的常量迭代器。
...
用于保存提取的容器元素的类，建模为 https://en.cppreference.com/w/cpp/container/node_handle[NodeHandle]。
...
使用 `hasher()` 作为哈希函数、`key_equal()` 作为键相等谓词、`allocator_type()` 作为分配器以及最大负载因子 `1.0` 构造一个空容器。
...
构造一个至少包含 `n` 个桶的空容器，使用 `hf` 作为哈希函数、`eql` 作为键相等谓词、`a` 作为分配器以及最大负载因子 `1.0`。
...
构造一个至少包含 `n` 个桶的空容器，使用 `hf` 作为哈希函数、`eql` 作为键相等谓词、`a` 作为分配器以及最大负载因子 `1.0`，并将 `[f, l)` 范围内的元素插入其中。
...
使用分配器 `a` 构造一个空容器，使用默认哈希函数和键相等谓词以及最大负载因子 `1.0`，并将 `[f, l)` 范围内的元素插入其中。
...
构造一个容器，移动 ``other`` 所包含的元素，并具有相同的哈希函数、谓词和最大负载因子，但使用分配器 `a`。
...
构造一个至少包含 `n` 个桶的空容器，使用 `hf` 作为哈希函数、`eql` 作为键相等谓词、`a` 作为分配器以及最大负载因子 `1.0`，并将 `il` 中的元素插入其中。
...
构造一个至少包含 `n` 个桶的空容器，使用默认哈希函数和键相等谓词、`a` 作为分配器以及最大负载因子 `1.0`。
...
构造一个至少包含 `n` 个桶的空容器，使用 `hf` 作为哈希函数、默认键相等谓词、`a` 作为分配器以及最大负载因子 `1.0`。
...
构造一个至少包含 `n` 个桶的空容器，使用分配器 `a`、默认哈希函数和键相等谓词以及最大负载因子 `1.0`，并将 `[f, l)` 范围内的元素插入其中。
...
构造一个至少包含 `n` 个桶的空容器，使用 `hf` 作为哈希函数、`a` 作为分配器、默认键相等谓词以及最大负载因子 `1.0`，并将 `[f, l)` 范围内的元素插入其中。
...
使用分配器 `a` 和最大负载因子 `1.0` 构造一个空容器，并将 `il` 中的元素插入其中。
...
构造一个至少包含 `n` 个桶的空容器，使用分配器 `a` 和最大负载因子 `1.0`，并将 `il` 中的元素插入其中。
...
构造一个至少包含 `n` 个桶的空容器，使用 `hf` 作为哈希函数、`a` 作为分配器以及最大负载因子 `1.0`，并将 `il` 中的元素插入其中。
...
[horizontal]
返回：;; 一个指向容器首元素的 `const_iterator`，若容器为空则为尾后值。
...
[horizontal]
要求：;; `value_type` 需满足从 `args` 到 `X` 的 https://en.cppreference.com/w/cpp/named_req/EmplaceConstructible[就地构造^]。
返回：;; 若插入发生，则返回类型的布尔分量为 true。+
+
若插入发生，则迭代器指向新插入的元素；否则指向具有等价键的元素。
抛出：;; 若除调用 `hasher` 外的操作抛出异常，则函数无效果。
说明：;; 可能使迭代器失效，但仅当插入导致负载因子大于或等于最大负载因子时。+
+
指向元素的指针和引用永不失效。+
+
...
`position` 是元素应插入位置的提示。

[horizontal]
要求：;; `value_type` 需满足从 `args` 到 `X` 的 https://en.cppreference.com/w/cpp/named_req/EmplaceConstructible[就地构造^]。
返回：;; 若插入发生，则迭代器指向新插入的元素；否则指向具有等价键的元素。
抛出：;; 若除调用 `hasher` 外的操作抛出异常，则函数无效果。
说明：;; 标准对提示的含义相当模糊。但唯一实用的使用方式（也是 Boost.Unordered 支持的唯一方式）是指向具有相同键的现有元素。+
+
...
[horizontal]
要求：;; `value_type` 需满足从 `*first` 到 `X` 的 https://en.cppreference.com/w/cpp/named_req/EmplaceConstructible[就地构造^]。
...
若插入发生，则迭代器指向新插入的元素；否则指向具有等价键的元素。
抛出：;; 若除调用 `hasher` 外的操作抛出异常，则函数无效果。
说明：;; 此函数类似于 xref:#unordered_map_emplace[emplace]，但 `value_type` 的构造使用：+
+
--
```c++
...
仅当 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员类型定义，且 `iterator` 和 `const_iterator` 均不能从 `K` 隐式转换时，`template<class K, class\... Args>` 重载才参与重载决议。库假定 `Hash` 可同时用 `K` 和 `Key` 调用，且 `Pred` 是透明的。这启用了异构查找，避免了实例化 `Key` 类型的开销。
--
...
返回：;; 若插入发生，则迭代器指向新插入的元素；否则指向具有等价键的元素。
抛出：;; 若除调用 `hasher` 外的操作抛出异常，则函数无效果。
说明：;; 此函数类似于 xref:#unordered_map_emplace_hint[emplace_hint]，但 `value_type` 的构造使用：+
+
--
```c++
...
说明：;; 使用此方法提取的节点可插入兼容的 `unordered_multimap`。
...
抛出：;; 仅当 `hasher` 或 `key_equal` 抛出异常时才会抛出异常。
说明：;; 在旧版本中，此操作可能效率低下，因为必须遍历多个桶以找到返回迭代器的位置。数据结构已更改，不再存在此问题，因此替代擦除方法已被弃用。
...
返回：;; 被擦除的元素数量。
抛出：;; 仅当 `hasher` 或 `key_equal` 抛出异常时才会抛出异常。
说明：;; 仅当 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员类型定义，且 `iterator` 和 `const_iterator` 均不能从 `K` 隐式转换时，`template<class K>` 重载才参与重载决议。库假定 `Hash` 可同时用 `K` 和 `Key` 调用，且 `Pred` 是透明的。这启用了异构查找，避免了实例化 `Key` 类型的开销。
...
抛出：;; 仅当 `hasher` 或 `key_equal` 抛出异常时才会抛出异常。+
+
在此实现中，此重载不调用任何函数对象的方法，因此不会抛出异常，但在其他实现中可能并非如此。
...
+
在此实现中，此重载不调用任何函数对象的方法，因此不会抛出异常，但在其他实现中可能并非如此。
说明：;; 此方法因从擦除操作返回后继迭代器代价高昂而实现，但容器已重新设计，不再存在此问题。因此此方法现已弃用。
...
说明：;; 异常规范与 C++11 标准不完全相同，因为等价谓词和哈希函数通过其复制构造函数交换。
...
由于 `source` 可能具有不同的哈希函数和键相等谓词，`source` 中每个节点的键会使用 `this\->hash_function()` 重新哈希，然后（如有必要）使用 `this\->key_eq()` 进行比较。
...
说明：;; 包含 `CompatibleKey`、`CompatibleHash` 和 `CompatiblePredicate` 的模板化重载是非标准扩展，允许使用兼容的哈希函数和等价谓词处理不同类型的键，以避免昂贵的类型转换。通常不鼓励使用此扩展，而应使用 `K` 成员函数模板。+
+
...
返回：;; 对 `x.second` 的引用，其中 `x` 是容器中已存在的元素，或是新插入的键等价于 `k` 的元素。
抛出：;; 若除调用 `hasher` 外的操作抛出异常，则函数无效果。
说明：;; 可能使迭代器失效，但仅当插入导致负载因子大于或等于最大负载因子时。+
+
指向元素的指针和引用永不失效。+
...
抛出：;; 若不存在此类元素，则抛出类型为 `std::out_of_range` 的异常对象。
说明：;; 仅当 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员类型定义时，`template<class K>` 重载才参与重载决议。库假定 `Hash` 可同时用 `K` 和 `Key` 调用，且 `Pred` 是透明的。这启用了异构查找，避免了实例化 `Key` 类型的开销。
...
[horizontal]
要求：;; `value_type` 需满足从 `(std::remove_const<key_type>::type&&, std::remove_const<mapped_type>::type&&)` 的 https://en.cppreference.com/w/cpp/named_req/EmplaceConstructible[就地构造^]。`x.key_equal()` 在功能上等价于 `other.key_equal()`。
说明：;; 若归档使用 Boost 1.84 之前的版本保存，则必须全局定义配置宏 `BOOST_UNORDERED_ENABLE_SERIALIZATION_COMPATIBILITY_V0` 以使此操作成功；否则将抛出异常。
...
使 `iterator`（`const_iterator`）`it` 指向原始 `iterator`（`const_iterator`）在归档（XML 归档）`ar` 读取的存储中恢复的位置。
