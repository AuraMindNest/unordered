...
说明：;; 可能使迭代器、指针和引用失效，但仅当插入导致负载超过最大负载时发生。+
+
...
若插入发生，则迭代器指向新插入的元素；否则指向具有等价键的元素。
抛出：;; 若除调用 `hasher` 外的操作抛出异常，则函数无效果。
说明：;; 此函数类似于 xref:#unordered_flat_map_emplace[emplace]，区别在于若存在等价键的元素则不构造 `value_type`；否则，构造形式如下：+
+
--
```c++
...
抛出：;; 若除调用 `hasher` 外的操作抛出异常，则函数无效果。
说明：;; 此函数类似于 xref:#unordered_flat_map_emplace_hint[emplace_hint]，区别在于若存在等价键的元素则不构造 `value_type`；否则，构造形式如下：+
+
--
```c++
...
若插入发生，则迭代器指向新插入的元素；否则指向具有等价键的元素。
抛出：;; 若除调用 `hasher` 外的操作抛出异常，则函数无效果。
说明：;; 可能使迭代器、指针和引用失效，但仅当插入导致负载超过最大负载时。+
+
...
抛出：;; 无。
说明：;; 返回的不透明对象必须立即丢弃或转换为 `iterator` 或 `const_iterator`。
...
说明：;; 构造、重哈希或清空后，容器的最大负载至少为 `max_load_factor() * bucket_count()`。在高负载条件下擦除元素时，此数值可能减少。
...
说明：;; 仅当 xref:reference/stats.adoc#stats[统计计算] 被 xref:unordered_flat_map_boost_unordered_enable_stats[启用] 时可用。
