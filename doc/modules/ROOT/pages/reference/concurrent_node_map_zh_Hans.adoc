...
|_Hash_
|一种一元函数对象类型，用作 `Key` 的哈希函数。它接受一个类型为 `Key` 的参数，并返回一个类型为 `std::size_t` 的值。

|_Pred_
|一种二元函数对象，它在类型 `Key` 的值上引入等价关系。它接受两个类型为 `Key` 的参数，并返回一个类型为 `bool` 的值。
...
[horizontal]
后置条件：;; `size() == 0`
要求：;; 若使用默认值，则 `hasher`、`key_equal` 和 `allocator_type` 需为 https://en.cppreference.com/w/cpp/named_req/DefaultConstructible[可默认构造(DefaultConstructible)^]。
...
[horizontal]
要求：;; `hasher`、`key_equal` 需为 https://en.cppreference.com/w/cpp/named_req/DefaultConstructible[可默认构造(DefaultConstructible)^]。
...
[horizontal]
后置条件：;; `size() == 0`
要求：;; `hasher` 和 `key_equal` 需为 https://en.cppreference.com/w/cpp/named_req/DefaultConstructible[可默认构造(DefaultConstructible)^]。
...
[horizontal]
后置条件：;; `size() == 0`
要求：;; `key_equal` 需为 https://en.cppreference.com/w/cpp/named_req/DefaultConstructible[可默认构造(DefaultConstructible)^]。
...
[horizontal]
要求：;; `value_type` 需为 https://en.cppreference.com/w/cpp/named_req/CopyInsertable[可复制插入(CopyInsertable)^]
...
返回：;; 若插入发生则为 `true`。+
并发性：;; 阻塞于 `*this` 的重哈希。
注意：;; 形式为 `insert(x)` 的调用（其中 `x` 可同等地转换为 `const value_type&` 和 `const init_type&`）不歧义，并选择 `init_type` 重载。
...
[horizontal]
要求：;; `value_type` 需为 https://en.cppreference.com/w/cpp/named_req/MoveInsertable[可移动插入(MoveInsertable)^]。
返回：;; 若插入发生则为 `true`。
并发性：;; 阻塞于 `*this` 的重哈希。
注意：;; 形式为 `insert(x)` 的调用（其中 `x` 可同等地转换为 `value_type&&` 和 `init_type&&`）不歧义，并选择 `init_type` 重载。
...
返回：;; 若插入发生则为 `true`。
并发性：;; 阻塞于 `*this` 的重哈希。
注意：;; 仅当 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员类型定义时，`template<class K, class M>` 才参与重载决议。库假定 `Hash` 可同时用 `K` 和 `Key` 调用，且 `Pred` 是透明的。这支持异构查找，避免了实例化 `Key` 类型的开销。
...
注意：;; 仅当 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员类型定义时，`template<class K>` 重载才参与重载决议。库假定 `Hash` 可同时用 `K` 和 `Key` 调用，且 `Pred` 是透明的。这支持异构查找，避免了实例化 `Key` 类型的开销。
...
[horizontal]
抛出：;; 除非 `key_equal` 或 `hasher` 在交换时抛出，否则无异常。
并发性：;; 阻塞于 `*this` 和 `other`。
...
若以下任意条件为真，则推导指引不参与重载决议：
  ...
  - 其具有 `InputIterator` 模板参数，且推导出该参数的类型不符合输入迭代器要求。
  - 其具有 `Allocator` 模板参数，且推导出该参数的类型不符合分配器要求。
  - 其具有 `Hash` 模板参数，且推导出该参数的类型为整型或符合分配器要求。
  - 其具有 `Pred` 模板参数，且推导出该参数的类型符合分配器要求。
...
[horizontal]
要求：;; `std::remove_const<key_type>::type` 和 `std::remove_const<mapped_type>::type` 必须是可序列化的（XML 可序列化），且支持 Boost.Serialization 的 `save_construct_data`/`load_construct_data` 协议（https://en.cppreference.com/w/cpp/named_req/DefaultConstructible[可默认构造(DefaultConstructible)^] 类型自动支持）。
并发性：;; 阻塞于 `x`。
