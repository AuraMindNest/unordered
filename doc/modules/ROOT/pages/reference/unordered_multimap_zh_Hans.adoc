...
返回：;; An iterator pointing to the inserted element.
...
说明：;; A node extracted using this method can be inserted into a compatible `unordered_map`.
...
[horizontal]
要求：;; The `unordered_multimap` `x` pointed to by `it` has been previously saved to `ar`, and no modifying operations have been issued on `x` between saving of `x` and saving of `it`.
...
[horizontal]
要求：;; If `x` is the `unordered_multimap` `it` points to, no modifying operations have been issued on `x` between loading of `x` and loading of `it`.
