...
如果 `link:../../../../../container_hash/doc/html/hash.html#ref_hash_is_avalanchinghash[hash_is_avalanching]<Hash>::value` 为 `true`，则直接使用哈希函数；否则，会添加一个位混合后处理阶段以提高哈希质量，但会牺牲额外的计算成本。
...
[horizontal]
后置条件：;; `size() == 0`
要求：;; 若使用默认值，则 `hasher`、`key_equal` 和 `allocator_type` 需满足 https://en.cppreference.com/w/cpp/named_req/DefaultConstructible[DefaultConstructible^]。
...
[horizontal]
要求：;; `hasher`、`key_equal` 需满足 https://en.cppreference.com/w/cpp/named_req/DefaultConstructible[DefaultConstructible^]。
...
[horizontal]
后置条件：;; `size() == 0`
要求：;; `hasher` 和 `key_equal` 需满足 https://en.cppreference.com/w/cpp/named_req/DefaultConstructible[DefaultConstructible^]。
...
[horizontal]
后置条件：;; `size() == 0`
要求：;; `key_equal` 需满足 https://en.cppreference.com/w/cpp/named_req/DefaultConstructible[DefaultConstructible^]。
...
赋值运算符。销毁先前存在的元素，从 `other` 拷贝赋值哈希函数和谓词，如果 `Alloc::propagate_on_container_copy_assignment` 存在且 `Alloc::propagate_on_container_copy_assignment::value` 为 `true`，则从 `other` 拷贝赋值分配器，最后插入 `other` 元素的副本。

[horizontal]
要求：;; `value_type` 需满足 https://en.cppreference.com/w/cpp/named_req/CopyInsertable[CopyInsertable^]
...
[horizontal]
要求：;; `value_type` 需满足 https://en.cppreference.com/w/cpp/named_req/MoveInsertable[MoveInsertable^]。
返回：;; 若插入发生则为 `true`。
并发性：;; 阻塞于 `*this` 的重哈希。
...
并发性：;; 阻塞于 `*this` 的重哈希。
注意：;; 若 `nh` 非空且 `nh` 的分配器与容器分配器不相等，则行为未定义。
...
[horizontal]
返回：;; 一个包含被提取元素的 `node_type` 对象，若未提取任何元素则为空。
抛出：;; 仅当 `hasher` 或 `key_equal` 抛出异常时才抛出异常。
注意：;; 仅当 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员类型定义时，`template<class K>` 重载才会参与重载决议。库假定 `Hash` 可同时被 `K` 和 `Key` 调用，且 `Pred` 是透明的。这支持异构查找，避免了实例化 `Key` 类型的开销。
...
将 `source` 中键尚未存在于 `*this` 的所有元素移动插入到 `*this`，并从 `source` 中擦除这些元素。
...
=== 推导指南
若以下任一条件成立，则推导指南不参与重载决议：

  - 它具有 `InputIterator` 模板参数，且推导出该参数的类型不符合输入迭代器要求。
  - 它具有 `Allocator` 模板参数，且推导出该参数的类型不符合分配器要求。
  - 它具有 `Hash` 模板参数，且推导出该参数的类型为整型或符合分配器要求。
  - 它具有 `Pred` 模板参数，且推导出该参数的类型符合分配器要求。
