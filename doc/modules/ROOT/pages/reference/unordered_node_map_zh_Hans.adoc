...
返回值;; 如果插入操作发生，则返回类型的 `bool` 分量为 `true` 。 +
+
若插入成功，则迭代器指向新插入的元素；否则，指向具有等价键的元素。
抛出;; 若异常由 `hasher` 调用之外的操作引发，则该函数不产生任何效果。
备注;; 可能会导致迭代器失效，但仅当插入操作导致负载因子大于或等于最大负载因子时才会发生。 +
+
...
[horizontal]
返回值;; 返回一个从 `position` 、 `inserted` 和 `node` 构造的 `insert++_++return++_++type` 对象： +
...
[horizontal]
返回值;; 若 `nh` 为空，则返回的迭代器为 `end()` 。若插入操作发生，则迭代器指向新插入的元素；否则，指向具有等价键的元素。
抛出;; 若异常由 `hasher` 调用之外的操作引发，则该函数不产生任何效果。
备注;; 如果 `nh` 非空，且 `nh` 与容器的分配器不相等，则行为未定义。
...
若插入成功，则迭代器指向新插入的元素；否则，指向具有等价键的元素。
抛出;; 若异常由 `hasher` 调用之外的操作引发，则该函数不产生任何效果。
备注;; 此函数与 xref:#unordered_node_map_emplace[emplace] 类似，区别在于：若已存在具有等价键的元素，则不会构造任何 `value++_++type` 对象；否则，将按以下形式构造： +
+
--
```c++
...
抛出;; 若异常由 `hasher` 调用之外的操作引发，则该函数不产生任何效果。
备注;; 此函数与 xref:#unordered_node_map_emplace_hint[emplace++_++hint] 类似，区别在于：若已存在具有等效键的元素，则不会构造任何 `value++_++type` 对象；否则，将按以下形式构造： +
+
--
```c++
...
若插入成功，则迭代器指向新插入的元素；否则，指向具有等价键的元素。
抛出;; 若异常由 `hasher` 调用之外的操作引发，则该函数不产生任何效果。
备注;; 可能会导致迭代器失效，但仅当插入操作导致负载超过最大负载时才会发生。 +
+
...
[horizontal]
返回值;; 返回一个不透明对象，该对象可隐式转换为擦除前紧接在 `position` 之后的 `iterator` 或 `const++_++iterator` 。
抛出;; 无。
备注;; 返回的不透明对象必须被立即丢弃或转换为 `iterator` 或 `const++_++iterator` 。
...
[horizontal]
返回值;; 返回一个包含被提取元素的 `node++_++type` 对象。
抛出;; 无。
...
备注;; 指向被转移元素的迭代器将失效。若 `++*++this` 的最终大小大于其原始最大负载，则与 `++*++this` 关联的所有迭代器将失效。
...
[horizontal]
返回值;; 返回一个指向键等价于 `k` 的元素的迭代器；若不存在这样的元素，则返回 `end()` 。
备注;; 仅当 `Hash::is++_++transparent` 与 `Pred::is++_++transparent` 均为有效的成员类型别名时， `template++<++class K++>++` 重载才会参与重载决议。标准库假定 `Hash` 可同时被 `K` 和 `Key` 类型调用，且 `Pred` 为透明谓词。该机制支持异构查找，从而避免实例化 `Key` 类型的开销。
...
返回值;; 返回一个引用，该引用指向容器中已存在的键等于 `k` 的元素 `x` 的 `x.second` 成员；若元素不存在，则指向新插入元素的该成员。
抛出;; 若异常由 `hasher` 调用之外的操作引发，则该函数不产生任何效果。
备注;; 可能会导致迭代器失效，但仅当插入操作导致负载因子大于或等于最大负载因子时才会发生。 +
+
...
[horizontal]
返回值;; 返回一个引用，该引用指向键等价于 `k` 的（唯一）元素 `x` 的 `x.second` 成员。
...
[horizontal]
返回值;; 返回容器在不重哈希的情况下可容纳的最大元素数（假设不再擦除元素）。
备注;; 在构造、重新哈希或清空操作后，容器的最大负载至少为 `max++_++load++_++factor() ++*++ bucket++_++count()` 。在高负载条件下执行擦除操作时，此数值可能会降低。
...
[horizontal]
返回值;; 返回容器直到目前已执行插入和查找操作的统计摘要。
备注;; 仅当 xref:reference/stats.adoc#stats[统计计算] 功能 xref:#unordered_node_map_boost_unordered_enable_stats[启用] 时可用。
