...
若存在键等价于 `k` 的元素 `x`，则以 `x` 的引用调用 `f`。若 `*this` 为常量，则该引用为常量引用。
...
对于范围 [`first`, `last`) 内的每个元素 `k`，若容器中存在键等价于 `k` 的元素 `x`，则以 `x` 的引用调用 `f`。若 `*this` 为常量，则该引用为常量引用。
...
依次以表中每个元素的引用调用 `f`。若 `*this` 为常量，则该引用为常量引用。
...
以表中每个元素的引用调用 `f`。若 `*this` 为常量，则该引用为常量引用。执行根据指定执行策略的语义并行化。
...
依次以表中每个元素的引用调用 `f`，直到 `f` 返回 `false` 或所有元素均被访问。若 `*this` 为常量，则该引用为常量引用。
...
以表中每个元素的引用调用 `f`，直到 `f` 返回 `false` 或所有元素均被访问。若 `*this` 为常量，则该引用为常量引用。执行根据指定执行策略的语义并行化。
...
+
若 `args...` 的形式为 `k,v`，则仅使用 `k` 参数检查，延迟构造完整对象直至确认需要插入元素。
...
注意：;; 若触发重哈希，则指向元素的指针和引用将失效。+
+
形式为 `insert(x)` 的调用（其中 `x` 可等价位转换为 `const value_type&` 和 `const init_type&`）不会产生歧义，并选择 `init_type` 重载。
...
注意：;; 若触发重哈希，则指向元素的指针和引用将失效。+
+
形式为 `insert(x)` 的调用（其中 `x` 可等价位转换为 `value_type&&` 和 `init_type&&`）不会产生歧义，并选择 `init_type` 重载。
...
若表中无等价键元素，则插入由 `args` 构造的对象；否则以等价元素的引用调用 `f`（若使用 `emplace_or_cvisit` 则为常量引用）。
...
当且仅当表中无等价键元素时插入 `obj`；否则以等价元素的引用调用 `f`（若使用 `*_cvisit` 重载则为常量引用）。
...
注意：;; 若触发重哈希，则指向元素的指针和引用将失效。+
+
在形式为 `insert_or_[c]visit(obj, f)` 的调用中，仅当 `std::remove_cv<std::remove_reference<decltype(obj)>::type>::type` 为 `value_type` 时，接受 `const value_type&` 参数的重载才参与重载决议。
...
注意：;; 若触发重哈希，则指向元素的指针和引用将失效。+
+
在形式为 `insert_or_[c]visit(obj, f)` 的调用中，仅当 `std::remove_reference<decltype(obj)>::type` 为 `value_type` 时，接受 `value_type&&` 参数的重载才参与重载决议。
...
若表中无等价键元素，则插入由 `args` 构造的对象，并以新创建元素的非常量引用调用 `f1`；否则以等价元素的引用调用 `f2`（若使用 `emplace_and_cvisit` 则为常量引用）。
...
当且仅当表中无等价键元素时插入 `obj`，并以新创建元素的非常量引用调用 `极1`；否则以等价元素的引用调用 `f2`（若使用 `*_cvisit` 重载则为常量引用）。
...
注意：;; 若触发重哈希，则指向元素的指针和引用将失效。+
+
在形式为 `insert_and_[c]visit(obj, f1, f2)` 的调用中，仅当 `std::remove_cv<std::remove_reference<decltype(obj)>::type>::type` 为 `value_type` 时，接受 `const value_type&` 参数的重载才参与重载决议。
...
注意：;; 若触发重哈希，则指向元素的指针和引用将失效。+
+
在形式为 `insert_and_[c]visit(obj, f1, f2)` 的调用中，仅当 `std::remove_reference<decltype(obj)>::type` 为 `value_type` 时，接受 `value_type&&` 参数的重载才参与重载决议。
...
若表中无键为 `k` 的元素，则插入由 `k` 和 `args` 构造的元素。
...
仅当 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员类型定义时，`template<class K, class\... Args>` 重载才参与重载决议。库假定 `Hash` 可同时用 `K` 和 `Key` 调用，且 `Pred` 是透明的。这支持异构查找，避免了实例化 `Key` 类型的开销。
...
若表中无键为 `k` 的元素，则插入由 `k` 和 `args` 构造的元素；否则以等价元素的引用调用 `f`（若使用 `*_cvisit` 重载则为常量引用）。

[horizontal]
返回：;; 若发生插入则返回 `true`。+
并发性：;; 阻塞于 `*this` 的重哈希过程。
注意：;; 若存在等价键元素则不构造 `value_type`；否则构造形式为：+
+
--
```c++
...
仅当 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员类型定义时，`template<class K, class\... Args, class F>` 重载才参与重载决议。库假定 `Hash` 可同时用 `K` 和 `Key` 调用，且 `Pred` 是透明的。这支持异构查找，避免了实例化 `Key` 类型的开销。
...
若表中无键为 `k` 的元素，则插入由 `k` 和 `args` 构造的元素，并以新创建元素的非常量引用调用 `f1`；否则以等价元素的引用调用 `f2`（若使用 `*_cvisit` 重载则为常量引用）。
...
仅当 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员类型定义时，`template<class K, class\... Args, class F1, class F2>` 重载才参与重载决议。库假定 `Hash` 可同时用 `K` 和 `Key` 调用，且 `Pred` 是透明的。这支持异构查找，避免了实例化 `Key` 类型的开销。
...
向表中插入新元素或通过赋值更新现有元素的包含值。

若存在键为 `k` 的元素，则通过赋值 `std::forward<M>(obj)` 更新它。

若无此元素，则将其作为以下形式添加到表中：```c++
...
注意：;; 若触发重哈希，则指向元素的指针和引用将失效。+
+
仅当 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员类型定义时，`template<class K, class M>` 重载才参与重载决议。库假定 `Hash` 可同时用 `K` 和 `Key` 调用，且 `Pred` 是透明的。这支持异构查找，避免了实例化 `Key` 类型的开销。
...
依次以表中每个元素的非常量引用调用 `f`，并擦除使 `f` 返回 `true` 的元素。
...
以表中每个元素的非常量引用调用 `f`，并擦除使 `f` 返回 `true` 的元素。执行根据指定执行策略的语义并行化。
...
[horizontal]
效果：;; 不执行任何操作，因为用户不允许更改此参数。保留以保持与 `boost::unordered_map` 的兼容性。
