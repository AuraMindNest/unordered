...
注意：;; 若触发重哈希，则指向元素的指针和引用将失效。+
+
形式为 `insert(x)` 的调用（其中 `x` 可同等地转换为 `const value_type&` 和 `const init_type&`）不歧义，并选择 `init_type` 重载。
...
注意：;; 若触发重哈希，则指向元素的指针和引用将失效。+
+
形式为 `insert(x)` 的调用（其中 `x` 可同等地转换为 `value_type&&` 和 `init_type&&`）不歧义，并选择 `init_type` 重载。
...
注意：;; 若触发重哈希，则指向元素的指针和引用将失效。+
+
此接口仅为说明性，因为 C++ 不允许在可变参数包后声明参数 `f1` 和 `f2`。
...
注意：;; 若触发重哈希，则指向元素的指针和引用将失效。+
+
仅当 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员类型定义时，`template<class K, class M>` 才参与重载决议。库假定 `Hash` 可同时用 `K` 和 `Key` 调用，且 `Pred` 是透明的。这支持异构查找，避免了实例化 `Key` 类型的开销。
