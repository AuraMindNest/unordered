...
返回：;; 若插入发生则为 `true`。+
并发性：;; 阻塞于 `*this` 的重哈希。
备注：;; 此函数与 xref:#concurrent_flat_map_emplace[emplace] 类似，区别在于若存在等价键元素则不构造 `value_type`；否则构造形式为：+
+
--
```c++
...
备注：;; 若触发重哈希，则指向元素的指针和引用将失效。+
+
仅当 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员类型定义时，`template<class K, class M>` 才参与重载决议。库假定 `Hash` 可同时用 `K` 和 `Key` 调用，且 `Pred` 是透明的。这启用了异构查找，避免了实例化 `Key` 类型的开销。
...
备注：;; 仅在 xref:reference/stats.adoc#stats[统计计算] 被 xref:concurrent_flat_map_boost_unordered_enable_stats[启用] 时可用。
