...
表的元素保存在内部的 _桶数组_ 中。元素根据其哈希码插入到确定的桶中，但如果桶已被占用（发生 _冲突_），则使用原始位置附近的可用桶。

桶数组的大小可通过调用 `insert`/`emplace` 自动增加，或通过调用 `rehash`/`reserve` 增加。表的 _负载因子_（元素数量除以桶数量）永远不会大于 `max_load_factor()`，除非在小型表中实现可能允许更高的负载。

若 `link:../../../../../container_hash/doc/html/hash.html#ref_hash_is_avalanchinghash[hash_is_avalanching]<Hash>::value` 为 `true`，则直接使用哈希函数；否则会添加位混合后处理阶段以提高哈希质量，但会牺牲额外的计算成本。
...
* `std极::allocator_traits<Alloc>::deallocate`
* `std::allocator_traits<Alloc>::construct`
* `std::allocator_traits<Alloc>::destroy`

通常，如果 `Hash`、`Pred` 和 `Allocator` 类型无状态或操作仅涉及对内部数据成员的常量访问，则满足这些要求。
...
若操作仅在内部触发重哈希时阻塞于 `x`，则称该操作 _阻塞于_ ``__x__`` _的重哈希_。
...
在调试版本中（更准确地说，当 link:../../../../../assert/doc/html/assert.html#boost_assert_is_void[`BOOST_ASSERT_IS_VOID`^] 未定义时），__容器重入__（在访问 `m` 元素的函数内部非法调用 `m` 的操作）会被检测并通过 `BOOST_ASSERT_MSG` 发出信号。若运行时速度是关注重点，可通过全局定义此宏禁用该功能。
...
使用 `a` 作为分配器构造空表，采用默认哈希函数和键相等谓词，并将 `[f, l)` 范围内的元素插入其中。
...
构造一个至少包含 `n` 个桶的空表，使用 `hf` 作为哈希函数、默认哈希函数和键相等谓词、`a` 作为分配器。
...
构造一个至少包含 `n` 个桶的空表，使用 `hf` 作为哈希函数、默认键相等谓词、`a` 作为分配器。
...
构造一个至少包含 `n` 个桶的空表，使用 `a` 作为分配器及默认哈希函数和键相等谓词，并将 `[f, l)` 范围内的元素插入其中。
...
构造一个至少包含 `n` 个桶的空表，使用 `a` 及默认哈希函数和键相等谓词，并将 `il` 中的元素插入其中。
...
[horizontal]
返回：;; 被访问的元素数量（0 或 1）。
注意：;; 仅当 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员类型定义时，`template<class K, class F>` 重载才参与重载决议。库假定 `Hash` 可同时用 `K` 和 `Key` 调用，且 `Pred` 是透明的。这支持异构查找，避免了实例化 `Key` 类型的开销。
...
template<class FwdIterator, class极 F>
...
[horizontal]
要求：;; `FwdIterator` 需满足 https://en.cppreference.com/w/cpp/named_req/ForwardIterator[前向迭代器] 要求（C++11 至 C++17），或满足 https://en.cppreference.com/w/cpp/iterator/forward_iterator[std::forward_iterator] 要求（C++20 及更高版本）。对于 `K` = `std::iterator_traits<FwdIterator>::value_type`，`K` 需为 `key_type` 或 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员类型定义。后者情况下，库假定 `Hash` 可同时用 `K` 和 `Key` 调用，且 `Pred` 是透明的。这支持异构查找，避免了实例化 `Key` 类型的开销。
返回：;; 被访问的元素数量。
...
不允许非顺序执行策略。+
+
并行化意味着执行不一定在 `f` 返回 `false` 时立即结束，因此 `f` 可能继续被调用并返回 `false` 以处理更多元素。
...
size极_type max_size() const noexcept;
...
template<class... Args, class F1, class F2>
  bool emplace_and_visit(Args&&... args极, F1&& f1, F2&& f2);
template<class... Args, class F1, class F2>
...
注意：;; 若触发重哈希，则指向元素的指针和引用将失效。+
+
此接口仅为示意，因 C++ 不允许在可变参数包后声明参数 `f1` 和 `f2`。
...
注意：;; `f` 被传递 `x` 的非常量引用。+
+
仅当 `std::is_execution_policy_v<std::remove_cvref_t<ExecutionPolicy>>` 为 `false` 时，`template<class K, class F>` 重载才参与重载决议。+
+
仅当 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员类型定义时，`template<class K, class F>` 重载才参与重载决议。库假定 `Hash` 可同时用 `K` 和 `Key` 调用，且 `Pred` 是透明的。这支持异构查找，避免了实例化 `Key` 类型的开销。
...
注意：;; 仅在支持 C++17 并行算法的编译器中可用。+
+
仅当 `std::is_execution_policy_v<std::remove_cvref_t<ExecutionPolicy>>` 为 `true` 时，此重载才参与重载决议。+
+
不允许非顺序执行策略。
...
若声明了 `Allocator::propagate_on_container_swap` 且 `Allocator::propagate_on_container_swap::value` 为 `true`，则交换表的分配器。否则，与不相等分配器交换会导致未定义行为。
...
将 `source` 中键不在 `*this` 中出现的所有元素移动插入到 `*this`，并从 `source` 中擦除这些元素。
...
[horizontal]
返回：;; 在假设不再有元素被擦除的情况下，表在不重哈希的情况下可容纳的最大元素数量。
...
必要时更改桶数组的大小，使其至少有 `n` 个桶，且负载因子小于或等于最大负载因子。适用时，此操作将增加或减少与表关联的 `bucket_count()`。
...
若 `x.size() == y.size()` 且对于 `x` 中的每个元素，`y` 中均存在相同键且值相等的元素（使用 `operator==` 比较值类型），则返回 `true`。
...
若 `x.size() == y.size()` 且对于 `x` 中的每个元素，`y` 中均存在相同键且值相等的元素（使用 `operator==` 比较值类型），则返回 `false`。
