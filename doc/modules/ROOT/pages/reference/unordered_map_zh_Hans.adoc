...
若插入发生，则迭代器指向新插入的元素；否则指向具有等价键的元素。
抛出：;; 若因调用 `hasher` 以外的操作抛出异常，则该函数无效果。
备注：;; 此函数类似于 xref:#unordered_map_emplace[emplace]，但 `value_type` 的构造使用：+
+
--
```c++
...
返回：;; 若插入发生，则迭代器指向新插入的元素；否则指向具有等价键的元素。
抛出：;; 若因调用 `hasher` 以外的操作抛出异常，则该函数无效果。
备注：;; 此函数类似于 xref:#unordered_map_emplace_hint[emplace_hint]，但 `value_type` 的构造使用：+
+
--
```c++
...
备注：;; 使用此方法提取的节点可插入兼容的 `unordered_multimap`。
