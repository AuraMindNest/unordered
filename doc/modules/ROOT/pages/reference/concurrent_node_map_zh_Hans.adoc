...
返回：;; 如果插入发生则返回 `true`。
并发性：;; 阻塞于 `*this` 的重哈希。
注意：;; 如果 `args...` 的形式为 `k,v`，则仅使用 `k` 参数进行检查，延迟构造整个对象直到确定应插入元素。
...
返回：;; 如果插入发生则返回 `true`。+
并发性：;; 阻塞于 `*this` 的重哈希。
注意：;; 形式为 `insert(x)` 的调用（其中 `x` 可同等地转换为 `const value_type&` 和 `const init_type&`）不会产生歧义，并选择 `init_type` 重载。
...
返回：;; 如果插入发生则返回 `true`。
并发性：;; 阻塞于 `*this` 的重哈希。
注意：;; 形式为 `insert(x)` 的调用（其中 `x` 可同等地转换为 `value_type&&` 和 `init_type&&`）不会产生歧义，并选择 `init_type` 重载。
...
如果存在键为 `k` 的元素，则通过赋值 `std::forward<M>(obj)` 更新它。
...
返回：;; 如果插入发生则返回 `true`。
并发性：;; 阻塞于 `*this` 的重哈希。
注意：;; 仅当 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员类型定义时，`template<class K, class M>` 才会参与重载解析。库假定 `Hash` 可同时用 `K` 和 `Key` 调用，且 `Pred` 是透明的。这支持异构查找，避免了实例化 `Key` 类型的开销。
...
+
仅当 `std::is_execution_policy_v<std::remove_cvref_t<ExecutionPolicy>>` 为 `false` 时，`template<class K, class F>` 重载才会参与重载解析。+
+
...
[horizontal]
效果：;; 不执行任何操作，因为不允许用户更改此参数。保留以兼容 `boost::unordered_map`。
...
==== __iter-to-alloc-type__（迭代器到分配器类型）
[listings, subs="+macros,+quotes"]
-----
template<class InputIterator>
...
[horizontal]
要求：;; `std::remove_const<key_type>::type` 和 `std::remove_const<mapped_type>::type` 必须是可序列化的（XML 可序列化），并且它们支持 Boost.Serialization 的 `save_construct_data`/`load_construct_data` 协议（由 https://en.cppreference.com/w/cpp/named_req/DefaultConstructible[可默认构造^] 类型自动支持）。
并发性：;; 阻塞于 `x`。
