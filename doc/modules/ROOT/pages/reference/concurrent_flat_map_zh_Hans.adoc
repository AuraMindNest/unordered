...
返回：;; 若插入发生则为 `true`。+
并发性：;; 阻塞于 `*this` 的重哈希。
说明：;; 此函数与 xref:#concurrent_flat_map_emplace[emplace] 类似，区别在于若存在等价键元素则不构造 `value_type`；否则构造形式为：+
+
--
```c++
...
说明：;; 仅在 xref:reference/stats.adoc#stats[统计计算] 被 xref:concurrent_flat_map_boost_unordered_enable_stats[启用] 时可用。
