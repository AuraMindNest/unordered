...
`boost::concurrent_flat_map` —— 一种哈希表，将唯一键与另一个值关联，并允许在无外部同步机制的情况下并发执行元素插入、擦除、查找和访问操作。

尽管 `boost::concurrent_flat_map` 充当容器，但它并不符合标准 C++ https://en.cppreference.com/w/cpp/named_req/Container[容器] 概念。特别地，它不提供迭代器及相关操作（如 `begin`、`end` 等）。元素的访问和修改通过用户提供的 _访问函数_ 完成，这些函数被传递给 `concurrent极_flat_map` 的操作，并在内部以受控方式执行。这种基于访问的 API 设计支持低争用的并发使用场景。

`boost::concurrent_flat_map` 的内部数据结构类似于 `boost::unordered_flat_map`。由于采用开放寻址技术，`value_type` 必须支持移动构造，且在重哈希过程中不保证指针稳定性。
...
namespace unordered {
  ...
  public:
    ...
    using stats                = xref:reference/stats.adoc#stats_stats_type[__统计类型__]; // 若统计功能被 xref:concurrent_flat_map_boost_unordered_enable_stats[启用]

    // 常量
    static constexpr size_type xref:#concurrent_flat_map_constants[bulk_visit_size] = _由实现定义_;
    ...
    // 修改器
    template<class... Args> bool xref:#concurrent_flat_map_emplace[emplace](Args&&... args);极
    ...
    template<class... Args> bool xref:#concurrent_flat_map_try_极place[try_emplace](key_type&& k, Args&&... args);
    ...
    // 统计（若 xref:concurrent_flat_map_boost_unordered_enable_stats[启用]）
  ...
  // 推导指南
  template<class InputIterator,
           class Hash = boost::hash<xref:#concurrent_flat_map_iter_key_type[__迭代器键类型__]<InputIterator>>,
           class Pred = std::equal_to<xref:#concurrent_flat_map_iter_key_type[__迭代器键类型__]<InputIterator>>,
           class Allocator = std::allocator<xref:#concurrent_flat_map_iter_to_alloc_type[__迭代器到分配类型__]<InputIterator>>>
    concurrent_flat_map(InputIterator, InputIterator, typename xref:#concurrent_flat_map_deduction_guides[__见下文__]::size_type = xref:#concurrent_flat_map_deduction_guides[__见下文__],
                        Hash = Hash(), Pred = Pred(), Allocator = Allocator())
      -> concurrent_flat_map<xref:#concurrent_flat_map_iter_key_type[__迭代器键类型__]<InputIterator>, xref:#concurrent_flat_map_iter_mapped_type[__迭代器映射类型__]<InputIterator>, Hash,
                             Pred, Allocator>;
  ...
  template<class Key, class T, class Hash = boost::hash<Key>,
    ...
    concurrent_flat_map(std::initializer_list<std::pair<Key, T>>,
                        typename xref:#concurrent_flat_map_deduction_guides[__见下文__]::size_type = xref:#concurrent_flat_map_deduction_guides[__见下文__], Hash = Hash(),
                        Pred = Pred(), Allocator = Allocator())
      -> concurrent_flat_map<Key, T, Hash, Pred, Allocator>;

  template<class InputIterator, class Allocator>
    concurrent_flat_map(InputIterator, InputIterator, typename xref:#concurrent_flat_map_deduction_guides[__见下文__]::size_type, Allocator)
      -> concurrent_flat_map<xref:#concurrent_flat_map_iter_key_type[__迭代器键类型__]<InputIterator>, xref:#concurrent_flat_map_iter_mapped_type[__迭代器映射类型__]<InputIterator>,
                             boost::hash<xref:#concurrent_flat_map_iter_key_type[__迭代器键类型__]<InputIterator>>,
                             std::equal_to<xref:#concurrent_flat_map_iter_key_type[__迭代器键类型__]<InputIterator>>, Allocator>;
  ...
  template<class InputIterator, class Hash, class Allocator>
    concurrent_flat_map(InputIterator, InputIterator, typename xref:#concurrent_flat_map_deduction_guides[__见下文__]::size_type, Hash,
                        Allocator)
      -> concurrent_flat_map<xref:#concurrent_flat_map_iter_key_type[__迭代器键类型__]<InputIterator>, xref:#concurrent_flat_map_iter_mapped_type[__迭代器映射类型__]<InputIterator>, Hash,
                             std::equal_to<xref:#concurrent_flat_map_iter_key_type[__迭代器键类型__]<InputIterator>>, Allocator>;

  template<class Key, class T, class All极ator>
    concurrent_flat_map(std::initializer_list<std::pair<Key, T>>, typename xref:#concurrent_flat_map_deduction_guides[__见下文__]::size_type,
                        Allocator)
      -> concurrent_flat_map<Key, T, boost::hash<Key>, std::equal_to<极>, Allocator>;
...
|_键类型（Key）_
.2+|`Key` 和 `T` 必须满足 https://en.cppreference.com/w/cpp/named_req/MoveConstructible[可移动构造] 要求。
`std::pair<const Key, T>` 必须能从任何可转换为它的 `std::pair` 对象 https://en.cppreference.com/w/cpp/named_req/EmplaceConstructible[原位构造] 到表中，并且必须能从表中 https://en.cppreference.com/w/cpp/named_req/Erasable[擦除]。
...
除析构操作外，在同一个 `concurrent_flat_map` 实例上并发调用任何操作均不会引入数据竞争——即它们是线程安全的。

若操作 *op* 被显式指定为 _阻塞于_ `x`（其中 `x` 是 `boost::concurrent_flat_map` 的实例），则 `x` 上先前的阻塞操作将与 *op* 同步。因此，同一 `concurrent_flat_map` 上的阻塞操作在多线程场景中会顺序执行。
...
任何插入或修改元素 `e` 的 `boost::concurrent_flat_map` 操作均与在 `e` 上内部调用的访问函数同步。

由 `boost::concurrent_flat_map` `x` 执行的访问函数不得调用 `x` 上的任何操作；允许调用不同 `boost::concurrent_flat_map` 实例 `y` 上的操作，前提是 `y` 上并发的未完成操作不会直接或间接访问 `x`。
...
在 xref:concurrent_flat_map_bulk_visit[批量访问] 操作内部使用的块大小。
...
移动构造函数。`other` 的内部桶数组直接转移至新表。哈希函数、谓词和分配器从 `other` 移动构造。若统计功能被 xref:concurrent_flat_map_boost_unordered_enable_stats[启用]，则从 `other` 转移内部统计信息并调用 `other.reset_stats()`。
...
template<class InputIterator>
  concurrent_flat_map(InputIterator f, InputIterator l, const allocator极& a);
...
若 `a == other.get_allocator()`，则 `other` 的元素直接转移至新表；否则元素从 `other` 移动构造。哈希函数和谓词从 `other` 移动构造，分配器从 `a` 复制构造。若统计功能被 xref:concurrent_flat_map_boost_unordered_enable_stats[启用]，则当且仅当 `a == other.get_allocator()` 时转移内部统计信息，并始终调用 `other.reset_stats()`。
...
从 xref:#unordered_flat_map[`unordered_flat_map`] 移动构造。`other` 的内部桶数组直接转移至新容器。哈希函数、谓词和分配器从 `other` 移动构造。若统计功能被 xref:concurrent_flat_map_boost_unordered_enable_stats[启用]，则转移内部统计信息并调用 `other.reset_stats()`。
...
若统计功能被 xref:concurrent_flat_map_boost_unordered_enable_stats[启用]，
...
尽管功能上等效于对每个键单独调用 xref:concurrent_flat_map_cvisit[`[c\]visit`]，但批量访问因内部流线优化通常更快。建议 `std::distance(first,last)` 至少为 xref:#concurrent_flat_map_constants[`bulk_visit_size`] 以获得性能提升：超过此大小后，性能预计不再提升。
...
+
如果 `args...` 的形式为 `k,v`，则仅使用 `k` 参数进行检查，延迟构造整个对象直到确定应插入元素。
...
注意：;; 若触发重哈希，则指向元素的指针和引用将失效。+
+
在形式为 `insert_or_[c]visit(obj, f)` 的调用中，仅当 `std::remove_cv<std::remove_reference<decltype(obj)>::type>::type` 是 `value_type` 时，接受 `const value_type&` 参数的重载才会参与重载解析。
...
注意：;; 若触发重哈希，则指向元素的指针和引用将失效。+
+
在形式为 `insert_or_[c]visit(obj, f)` 的调用中，仅当 `std::remove_reference<decltype(obj)>::type` 是 `value_type` 时，接受 `value_type&&` 参数的重载才会参与重载解析。
...
注意：;; 若触发重哈希，则指向元素的指针和引用将失效。+
+
在形式为 `insert_and_[c]visit(obj, f1, f2)` 的调用中，仅当 `std::remove_cv<std::remove_reference<decltype(obj)>::type>::type` 是 `value_type` 时，接受 `const value_type&` 参数的重载才会参与重载解析。
...
注意：;; 若触发重哈希，则指向元素的指针和引用将失效。+
+
在形式为 `insert_and_[c]visit(obj, f1, f2)` 的调用中，仅当 `std::remove_reference<decltype(obj)>::type` 是 `value_type` 时，接受 `value_type&&` 参数的重载才会参与重载解析。
...
返回：;; 如果插入发生则返回 `true`。+
并发性：;; 阻塞于 `*this` 的重哈希。
注意：;; 此函数与 xref:#concurrent_flat_map_emplace[emplace] 类似，区别在于若存在等价键元素则不构造 `value_type`；否则构造形式为：+
+
--
```c++
...
而 xref:#concurrent_flat_map_emplace[emplace] 简单地将所有参数转发给 ``value_type`` 的构造函数。

若触发重哈希，则指向元素的指针和引用将失效。
...
注意：;; 若触发重哈希，则指向元素的指针和引用将失效。+
+
仅当 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员类型定义时，`template<class K, class M>` 才会参与重载解析。库假定 `Hash` 可同时用 `K` 和 `Key` 调用，且 `Pred` 是透明的。这支持异构查找，避免了实例化 `Key` 类型的开销。
...
注意：;; 仅在 xref:reference/stats.adoc#stats[统计计算] 被 xref:concurrent_flat_map_boost_unordered_enable_stats[启用] 时可用。
...
template<class InputIterator>
  using __iter-key-type__ = std::remove_const_t<
    std::tuple_element_t<0, xref:#concurrent_flat_map_iter_value_type[__iter-value-type__]<InputIterator>>>; // 仅用于说明
-----
...
template<class InputIterator>
  using __iter-mapped-type__ =
    std::tuple_element_t<1, xref:#concurrent_flat_map_iter_value_type[__iter-value-type__]<InputIterator>>;  // 仅用于说明
-----
...
template<class InputIterator>
  using __iter-to-alloc-type__ = std::pair<
    ...
    std::tuple_element极<1, xref:#concurrent_flat_map_iter_value_type[__iter-value-type__]<InputIterator>>>; // 仅用于说明
-----
...
``concurrent_flat_map`` 可通过 link:../../../../../serialization/index.html[Boost.Serialization^] 使用本库提供的 API 进行归档/检索。支持常规归档和 XML 归档。

==== 将 concurrent_flat_map 保存到归档

将 `concurrent_flat_map` `x` 的所有元素保存到归档（XML 归档）`ar` 中。
...
==== 从归档加载 concurrent_flat_map

删除 `concurrent_flat_map` `x` 的所有预先存在的元素，并从归档（XML 归档）`ar` 中插入原始 `concurrent_flat_map` `other` 保存到 `ar` 所读取存储中的元素恢复副本。
