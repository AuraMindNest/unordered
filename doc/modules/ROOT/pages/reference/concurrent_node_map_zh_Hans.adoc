...
[horizontal]
返回：;; 被访问的元素数量（0 或 1）。
说明：;; 仅当 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员类型定义时，`template<class K, class F>` 重载才会参与重载决议。库假定 `Hash` 可同时被 `K` 和 `Key` 调用，且 `Pred` 是透明的。这支持异构查找，避免了实例化 `Key` 类型的开销。
...
[horizontal]
说明：;; 在存在并发插入操作的情况下，返回值可能无法准确反映执行后表的实际大小。
...
返回：;; 若插入发生则为 `true`。
并发性：;; 阻塞于 `*this` 的重哈希。
说明：;; 若 `args...` 的形式为 `k,v`，则仅使用 `k` 参数检查，延迟构造整个对象直到确定应插入元素。
...
返回：;; 若插入发生则为 `true`。+
并发性：;; 阻塞于 `*this` 的重哈希。
说明：;; 形式为 `insert(x)` 的调用（其中 `x` 可同等地转换为 `const value_type&` 和 `const init_type&`）不歧义，并选择 `init_type` 重载。
...
返回：;; 若插入发生则为 `true`。
并发性：;; 阻塞于 `*this` 的重哈希。
说明：;; 形式为 `insert(x)` 的调用（其中 `x` 可同等地转换为 `value_type&&` 和 `init_type&&`）不歧义，并选择 `init_type` 重载。
...
并发性：;; 阻塞于 `*this` 的重哈希。
说明：;; 若 `nh` 非空且 `nh` 的分配器与容器分配器不相等，则行为未定义。
...
返回：;; 若插入发生则为 `true`。
并发性：;; 阻塞于 `*this` 的重哈希。
说明：;; 此接口仅为说明性，因为 C++ 不允许在可变参数包后声明参数 `f`。
...
返回：;; 若插入发生则为 `true`。+
并发性：;; 阻塞于 `*this` 的重哈希。
说明：;; 在形式为 `insert_or_[c]visit(obj, f)` 的调用中，仅当 `std::remove_cv<std::remove_reference<decltype(obj)>::type>::type` 是 `value_type` 时，接受 `const value_type&` 参数的重载才会参与重载解析。
...
返回：;; 若插入发生则为 `true`。+
并发性：;; 阻塞于 `*this` 的重哈希。
说明：;; 在形式为 `insert_or_[c]visit(obj, f)` 的调用中，仅当 `std::remove_reference<decltype(obj)>::type` 是 `value_type` 时，接受 `value_type&&` 参数的重载才会参与重载解析。
...
返回：;; 若插入发生则为 `true`。
并发性：;; 阻塞于 `*this` 的重哈希。
说明：;; 此接口仅为说明性，因为 C++ 不允许在可变参数包后声明参数 `f1` 和 `f2`。
...
返回：;; 若插入发生则为 `true`。+
并发性：;; 阻塞于 `*this` 的重哈希。
说明：;; 在形式为 `insert_and_[c]visit(obj, f1, f2)` 的调用中，仅当 `std::remove_cv<std::remove_reference<decltype(obj)>::type>::type` 是 `value_type` 时，接受 `const value_type&` 参数的重载才会参与重载解析。
...
返回：;; 若插入发生则为 `true`。+
并发性：;; 阻塞于 `*this` 的重哈希。
说明：;; 在形式为 `insert_and_[c]visit(obj, f1, f2)` 的调用中，仅当 `std::remove_reference<decltype(obj)>::type` 是 `value_type` 时，接受 `value_type&&` 参数的重载才会参与重载解析。
...
返回：;; 若插入发生则为 `true`。+
并发性：;; 阻塞于 `*this` 的重哈希。
说明：;; 此函数类似于 xref:#concurrent_node_map_emplace[emplace]，不同之处在于：如果存在键等价的元素，则不构造 `value_type`；否则，构造形式为：+
+
--
```c++
...
返回：;; 若插入发生则为 `true`。+
并发性：;; 阻塞于 `*this` 的重哈希。
说明：;; 如果存在键等价的元素，则不构造 `value_type`；否则，构造形式为：+
+
--
```c++
...
返回：;; 若插入发生则为 `true`。
并发性：;; 阻塞于 `*this` 的重哈希。
说明：;; 仅当 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员类型定义时，`template<class K, class M>` 才参与重载决议。库假定 `Hash` 可同时用 `K` 和 `Key` 调用，且 `Pred` 是透明的。这启用了异构查找，避免了实例化 `Key` 类型的开销。
...
说明：;; 仅当 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员类型定义时，`template<class K>` 重载才参与重载决议。库假定 `Hash` 可同时用 `K` 和 `Key` 调用，且 `Pred` 是透明的。这启用了异构查找，避免了实例化 `Key` 类型的开销。
...
说明：;; 仅当 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员类型定义时，`template<class K>` 重载才会参与重载决议。库假定 `Hash` 可同时被 `K` 和 `Key` 调用，且 `Pred` 是透明的。这支持异构查找，避免了实例化 `Key` 类型的开销。+
+
...
说明：;; 构造、重哈希或清除后，表的最大负载至少为 `max_load_factor() * bucket_count()`。在高负载条件下擦除元素时，此数值可能减少。+
+
在存在并发插入操作的情况下，返回值可能无法准确反映执行后表的实际状态。
...
说明：;; 仅当启用了 xref:reference/stats.adoc#stats[统计计算] 时可用。
...
=== 推导指南
若以下任一条件为真，则推导指南不参与重载决议：

  - 其具有 `InputIterator` 模板参数，且为该参数推导出的类型不符合输入迭代器要求。
  - 其具有 `Allocator` 模板参数，且为该参数推导出的类型不符合分配器要求。
  - 其具有 `Hash` 模板参数，且为该参数推导出的类型为整数类型或符合分配器要求。
  - 其具有 `Pred` 模板参数，且为该参数推导出的类型符合分配器要求。
...
[horizontal]
要求：;; `std::remove_const<key_type>::type` 和 `std::remove_const<mapped_type>::type` 需可序列化（XML 可序列化），且支持 Boost.Serialization 的 `save_construct_data`/`load_construct_data` 协议（https://en.cppreference.com/w/cpp/named_req/DefaultConstructible[可默认构造^] 类型自动支持）。
并发性：;; 阻塞于 `x`。
