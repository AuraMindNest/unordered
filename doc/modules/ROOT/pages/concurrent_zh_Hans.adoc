...
Boost.Unordered 提供了 `boost::concurrent_node_set`、`boost::concurrent_node_map`、`boost::concurrent_flat_set` 和 `boost::concurrent_flat_map`，这些哈希表允许不同线程进行并发读写访问，而无需用户端实现任何同步机制。
...
在多线程场景中，若其他线程在 A 和 B 之间执行了 `m.erase(k)` 操作，迭代器 `it` 在 B 点可能已失效。虽然存在通过锁定元素来修复此问题的设计，但这种方法易引发高争用，并可能导致程序死锁。`operator[]` 也存在类似的并发问题，因此 `boost::concurrent_flat_map`/`boost::concurrent_node_map` 也未提供该操作。相反，元素访问通过所谓的_访问函数_实现：
...
用户传递的访问函数（此处为 lambda 函数）由 Boost.Unordered 在内部以线程安全方式执行，因此可在访问元素时无需担心其他线程干扰。
...
访问机制在并发容器的 API 中占据重要地位，许多经典操作都提供了支持访问的变体：
...
注意在此最后示例中，访问函数实际上可_修改_元素：通常，对非常量并发映射 `m` 的操作会授予访问函数对元素的非常量访问权限。若需显式请求常量访问，可使用 `cvisit` 重载（如 `insert_or_cvisit`），这可能提高并行性。而对于并发集合，访问始终为常量访问。

虽然预期使用频率较低，但并发容器还提供在元素创建后立即访问的插入操作（除存在等价元素时的常规访问外）：
...
此功能不仅为语法便利：通过一次性处理所有键，可应用某些内部优化以提升性能（参阅 xref:benchmarks.adoc#benchmarks_boostconcurrent_flatnode_map[基准测试]）。实际上，缓冲传入键以分块批量访问可能更有利：
...
这存在延迟/吞吐量的权衡：传入键的处理时间更长（因需缓冲），但每秒处理键数更高。`bulk_visit_size` 是推荐的块大小——更小的缓冲区可能导致性能下降。

== 阻塞操作

并发容器可像其他 Boost.Unordered 容器一样复制、赋值、清空和合并。与大多数操作不同，这些是_阻塞_操作，即在复制、赋值、清空或合并过程中，其他线程无法访问相关表。阻塞由库自动处理，用户无需特别防范，但整体性能可能受影响。

另一阻塞操作是_重哈希_，可通过 `rehash`/`reserve` 显式触发，或在插入时表的负载达到 `max_load()` 时发生。与非并发容器类似，在批量插入前预留空间通常可加速过程。

== 与非并发容器的互操作性

由于开放寻址容器和并发容器基于相同的内部数据结构，它们可高效地通过移动构造从其非并发对应容器转换而来，反之亦然。
...
这种互操作性在多阶段场景中非常实用：部分数据处理并行执行，而其他步骤则非并发（或非修改）。在以下示例中，我们需要从庞大的单词输入向量构建直方图：填充阶段可使用 `boost::concurrent_flat_map` 并行完成，然后将结果转移到最终容器。
