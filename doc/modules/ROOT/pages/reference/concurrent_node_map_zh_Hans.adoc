...
|_键_
.2+|`std::pair++<++const Key, T++>++` 必须能够从任何可转换为该类型的
`std::pair`
对象 https://en.cppreference.com/w/cpp/named_req/EmplaceConstructible[原地构造] 到容器中，并且必须支持从容器中 https://en.cppreference.com/w/cpp/named_req/Erasable[擦除] 。
...
在 xref:concurrent_node_map_bulk_visit[批量访问] 操作中内部使用的数据块大小。
...
移动构造函数。 `other` 的内部桶数组直接转移至新容器。哈希函数、谓词和分配器从 `other` 移动构造。若统计功能 xref:#concurrent_node_map_boost_unordered_enable_stats[已启用] ，则同步转移 `other` 的内部统计信息并调用 `other.reset++_++stats()` 。
...
若 `a == other.get++_++allocator()` ，则 `other` 的元素直接转移至新容器；否则，元素从 `other` 移动构造。哈希函数和谓词从 `other` 移动构造，分配器从 `a` 复制构造。若统计功能 xref:#concurrent_node_map_boost_unordered_enable_stats[已启用] ，则当 `a == other.get++_++allocator()` 时同步转移 `other` 的内部统计信息，并始终调用 `other.reset++_++stats()` 。
...
从 xref:#unordered_node_map[`unordered_node_map`][`unordered++_++node++_++map`] 进行移动构造。 `other` 的内部桶数组直接转移至新容器。哈希函数、谓词和分配器从 `other` 移动构造。若统计功能 xref:#concurrent_node_map_boost_unordered_enable_stats[已启用] ，则同时转移 `other` 的内部统计信息并调用 `other.reset++_++stats()` 。
...
`++*++this` ；否则，插入 `other`
元素的移动构造副本。若统计功能 xref:#concurrent_node_map_boost_unordered_enable_stats[已启用] ，则当最终分配器等于
...
尽管在功能上等效于对每个键单独调用 xref:#concurrent_node_map_cvisit[`++[++c++]++visit`] ，但批量访问因内部流线型优化而通常具有更快的执行速度。建议 `std::distance(first,last)` 至少为 xref:#concurrent_node_map_constants[`bulk++_++visit++_++size`] 以获得性能提升：超过此大小，性能预计不会进一步提升。
...
并发性;; 阻塞于 `++*++this` 的重哈希。
备注;; 若 `args…` 的格式为 `k,v` ，该实现会延迟构造完整对象，直到确认需要插入元素时为止，在此期间仅使用参数 `k` 进行检查。
...
返回值;; 若插入操作发生，则返回 `true` 。 +
并发性;; 阻塞于 `++*++this` 的重哈希。
备注;; 对于 `insert(x)` 形式的调用（其中 `x` 可同等地转换为 `const value++_++type&` 和 `const init++_++type&` ），该调用不会产生歧义，并选择 `init++_++type` 重载。
...
返回值;; 若插入操作发生，则返回 `true` 。
并发性;; 阻塞于 `++*++this` 的重哈希。
备注;; 对于 `insert(x)` 形式的调用（其中 `x` 可同等地转换为 `const value++_++type&` 和 `const init++_++type&` ），不会产生歧义，并选择 `init++_++type` 重载。
...
[horizontal]
返回值;; 返回一个基于 `inserted` 和 `node` 构造的 `insert++_++return++_++type` 对象： +
...
并发性;; 阻塞于 `++*++this` 的重哈希。
备注;; 如果 `nh` 非空，且 `nh` 与容器的分配器不相等，则行为未定义。
...
返回值;; 若插入操作发生，则返回 `true` 。
并发性;; 阻塞于 `++*++this` 的重哈希。
备注;; 此接口仅为说明，因 C{plus}{plus} 不允许在可变参数包后声明参数 `f` 。
...
返回值;; 若插入操作发生，则返回 `true` 。 +
并发性;; 阻塞于 `++*++this` 的重哈希。
备注;; 在 `insert++_++or++_[++c++]++visit(obj, f)` 形式的调用中，仅当 `std::remove++_++cv++<++std::remove++_++reference++<++decltype(obj)++>++::type++>++::type` 为 `value++_++type` 时，接受 `const value++_++type&` 参数的重载才会参与重载决议。
...
返回值;; 若插入操作发生，则返回 `true` 。 +
并发性;; 阻塞于 `++*++this` 的重哈希。
备注;; 在 `insert++_++or++_[++c++]++visit(obj, f)` 形式的调用中，仅当 `std::remove++_++reference++<++decltype(obj)++>++::type` 为 `value++_++type` 时，接受 `value++_++type&&` 参数的重载才会参与重载决议。
...
返回值;; 若插入操作发生，则返回 `true` 。
并发性;; 阻塞于 `++*++this` 的重哈希。
备注;; 此接口仅为说明，因为 C{plus}{plus} 不允许在可变参数包后声明参数 `f1` 和 `f2` 。
...
返回值;; 若插入操作发生，则返回 `true` 。 +
并发性;; 阻塞于 `++*++this` 的重哈希。
备注;; 在 `insert++_++and++_[++c++]++visit(obj, f1, f2)` 形式的调用中，仅当 `std::remove++_++cv++<++std::remove++_++reference++<++decltype(obj)++>++::type++>++::type` 为 `value++_++type` 时，接受 `const value++_++type&` 参数的重载才会参与重载决议。
...
返回值;; 若插入操作发生，则返回 `true` 。 +
并发性;; 阻塞于 `++*++this` 的重哈希。
备注;; 在 `insert++_++and++_[++c++]++visit(obj, f1, f2)` 形式的调用中，仅当 `std::remove++_++reference++<++decltype(obj)++>++::type` 为 `value++_++type` 时，接受 `value++_++type&&` 参数的重载才会参与重载决议。
...
返回值;; 若插入操作发生，则返回 `true` 。 +
并发性;; 阻塞于 `++*++this` 的重哈希。
备注;; 此函数与 xref:#concurrent_node_map_emplace[emplace] 类似，区别在于：若存在等价键的元素，则不会构造任何 `value++_++type` ；否则，将按以下形式执行构造： +
+
--
```c++
...
与 xref:#concurrent_node_map_emplace[emplace] 不同——该操作仅将所有参数直接转发给 `value++_++type` 的构造函数。
...
返回值;; 若插入操作发生，则返回 `true` 。
并发性;; 阻塞于 `++*++this` 的重哈希。
备注;; 仅当 `Hash::is++_++transparent` 与 `Pred::is++_++transparent` 均为有效的成员类型别名时， `template++<++class K, class M++>++` 才会参与重载决议。该库假定 `Hash` 可同时被 `K` 和 `Key` 调用，且 `Pred` 为透明的。该机制支持异构查找，从而避免实例化 `Key` 类型对象所需的开销。
...
[horizontal]
返回值;; 返回一个包含被提取元素的 `node++_++type` 对象；若未提取到元素，则返回空对象。
抛出;; 仅当 `hasher` 或 `key++_++equal` 抛出异常时，本函数才会抛出异常。
备注;; 仅当 `Hash::is++_++transparent` 与 `Pred::is++_++transparent` 均为有效的成员类型别名时， `template++<++class K++>++` 重载才会参与重载决议。标准库假定 `Hash` 可同时被 `K` 和 `Key` 类型调用，且 `Pred` 为透明谓词。该机制支持异构查找，从而避免实例化 `Key` 类型的开销。
...
[horizontal]
返回值;; 返回一个包含被提取元素的 `node++_++type` 对象；若未提取到元素，则返回空对象。
抛出;; 仅当 `hasher` 、 `key++_++equal` 或 `f` 抛出异常时，才会抛出异常。
备注;; 仅当 `Hash::is++_++transparent` 与 `Pred::is++_++transparent` 均为有效的成员类型别名时， `template++<++class K++>++` 重载才会参与重载决议。标准库假定 `Hash` 可同时被 `K` 和 `Key` 类型调用，且 `Pred` 为透明谓词。该机制支持异构查找，从而避免实例化 `Key` 类型的开销。
...
备注;; 仅在 xref:reference/stats.adoc#stats[统计计算] 功能 xref:#concurrent_node_map_boost_unordered_enable_stats[启用] 时可用。
