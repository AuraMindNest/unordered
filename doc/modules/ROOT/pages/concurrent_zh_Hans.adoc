...
[[content]]
[[preamble]]
Boost.Unordered 提供 `boost::concurrent++_++node++_++set` 、 `boost::concurrent++_++node++_++map` 、 `boost::concurrent++_++flat++_++set` 和 `boost::concurrent++_++flat++_++map` ，这些哈希表允许不同线程进行并发读写访问，无需用户实现任何同步机制。
...
支持访问功能的完整操作列表请参阅 link:reference/concurrent_node_set_zh_Hans.adoc#concurrent_node_set[`boost::concurrent++_++node++_++set`]、link:reference/concurrent_node_map_zh_Hans.adoc#concurrent_node_map[`boost::concurrent++_++node++_++map`]、link:reference/concurrent_flat_set_zh_Hans.adoc#concurrent_flat_set[`boost::concurrent++_++flat++_++set`] 和 link:reference/concurrent_flat_map_zh_Hans.adoc#concurrent_flat_map[`boost::concurrent++_++flat++_++map`] 的参考文档。
...
|===
| `boost::concurrent++_++node++_++set` | `boost::unordered++_++node++_++set`
| `boost::concurrent++_++node++_++map` | `boost::unordered++_++node++_++map`
| `boost::concurrent++_++flat++_++set` | `boost::unordered++_++flat++_++set`
| `boost::concurrent++_++flat++_++map` | `boost::unordered++_++flat++_++map`
|===
